...
-(void) addTile: (TileNode*)tile row:(int)row col:(int)col {
...
//    NSLog(@"Wall.transitionUpWithTarget");
    ...
    [[OALSimpleAudio sharedInstance] playEffect:@"wallUp.m4a"];
...
//    NSLog(@"Wall.transitionUpWithTarget needToMoveUpCount starts at %d", self.needToMoveUpCount);
    ...
    for (NSUInteger  index = 0, len = [tiles count]; index < len; index++) {
        ...
        id actionMoveDone = [CCActionCallFunc actionWithTarget:self selector:@selector(tileUpDone)];

        if (tile && target && index == len - 1) {
//            NSLog(@"Wall.transitionUpWithTarget tile = %@, index = len - 1 = %d", tile, len - 1);
            id actionMoveFinal = [CCActionCallFunc actionWithTarget:target selector:callback];

            [tile runAction:[CCActionSequence actions:actionMove, actionMoveDone, actionMoveFinal, nil]];

        } else if (tile) {
//            NSLog(@"Wall.transitionUpWithTarget else if tile = %@", tile);
            
            [tile runAction:[CCActionSequence actions:actionMove, actionMoveDone, nil]];
            
        } else if (!tile && index == len - 1){
//            NSLog(@"Wall.transitionUpWithTarget else");

            id actionMoveFinal = [CCActionCallFunc actionWithTarget:target selector:callback];
            
            [tile runAction:[CCActionSequence actions:actionMoveFinal, nil]];
...
//    NSLog(@"Wall.removeTile tile released");
    ...
    for (int i = row + 1; i < MAX_ROWS; i++) {
        ...
        if ([tileRow objectAtIndex:col] != [NSNull null]) {
            TileNode* tile = [tileRow objectAtIndex:col];
            tile.row--;
             ...
             */
            id actionMove = [CCActionMoveTo actionWithDuration:TILE_DROP_TIME position:[self getPointInGrid:tile row:tile.row col:tile.col]];
            id actionMoveDone = [CCActionCallFunc actionWithTarget:self selector:@selector(snapAllToGrid)];
            [tile runAction:[CCActionSequence actions:actionMove, actionMoveDone, nil]];
    ...
    for (int i = 0; i < MAX_ROWS; i++) {
        ...
        for (int j = 0; j < MAX_COLS; j++) {
            if ([tileRow objectAtIndex:j] != [NSNull null]) {
                ...
                float x = tile.position.x - tile.contentSize.width / 2;
                float y = tile.position.y - tile.contentSize.height / 2;
                
                CGRect touchArea = CGRectMake(x, y, tile.contentSize.width, tile.contentSize.height);
...
-(CGPoint) getPointAtopTile:(TileNode*) tileToAdd referenceTile:(TileNode*)refTile {
    int col = refTile.col;
...
-(CGPoint) getPointInEmptySpot:(TileNode*) tileToAdd location:(CGPoint)location {
    /*
     * Find the column and row that the location is closest to
     */
    int col = -1;
    int width = tileToAdd.contentSize.width;
    int height = tileToAdd.contentSize.height;
    
    for (int j = 0; j < MAX_COLS; j++) {
        float minX = width * j;
        float maxX = width * (j + 1);
...
-(CGPoint) addTileAtopTile:(TileNode*) tileToAdd referenceTile:(TileNode*)refTile {
    int col = refTile.col;
    for (int i = refTile.row + 1; i < MAX_ROWS; i++) {
        if ([self isEmptyAtRow:i col:col]) {
            ...
            float x = refTile.contentSize.width * (col + 0.5);
            float y = refTile.contentSize.height * (i - 0.5);
...
-(CGPoint) getPointInGrid:(TileNode*) tile row:(int)row col:(int)col {
    int width = tile.contentSize.width;
    int height = tile.contentSize.height;
    
    float x = width * (col + 0.5);
    float y = height * (row - 0.5);
